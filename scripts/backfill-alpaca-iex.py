    ...
    def insert_bars(self, symbol_id: int, bars: list):
        ...
        data = [
            (
                ...
                float(bar['c']),
                int(bar['v']),
                int(bar.get('n', 0)),  # trade count
                float(bar.get('vw', bar['c']))  # VWAP
            )
            for bar in bars
        ]
        ...
        with self.conn.cursor() as cur:
            execute_batch(
                cur,
                """
                INSERT INTO candles (symbol_id, time, open, high, low, close, volume, trade_count, vwap)
...
def main():
    ...
    parser.add_argument('--years', type=int, default=1, help='Years of history to download (default: 1)')
